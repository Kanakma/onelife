...
class TeacherAddNewHomework extends React.Component {
  ...
  addHomework(event){
    ...
    // })
    //   .then(res => {
    //     var homework_id = res.data._id;
    //     return new Promise((resolve, reject) => {
    //       let fileFormData = new FormData();
    //       fileFormData.append('file', this.state.file);
    //       fileFormData.append('lesson', this.state.lesson);
    //       fileFormData.append('file', this.state.file);
    //       axios.post('/api/addhomeworkfile?homework_id='+homework_id, fileFormData, {
    //         responseType: 'json',
    //         headers: {
    //         'Content-type': 'application/x-www-form-urlencoded'
    //         }
        ...
        <form action="/"  onSubmit={this.addHomework}>
          <div className="form-group col-md-6">
            ...
            <div className="row" style={{textAlign: 'center', marginBottom: '20px'}}>
              <textarea maxLength="500" type="text" placeholder="Описание задания" rows="6" className="homework-message" onChange={this.handleChange}></textarea>
            </div>
